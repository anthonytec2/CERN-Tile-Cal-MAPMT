...
labels={'Single Anode PMT'};%Creates labels array for legened
    ...
    maxVal=max(holdData);
    for i=1:length(cpp(1,1,:))
        text(pks(i)-3,maxVal+10,[int2str(writeText(2*i-1)) ',' int2str(writeText(2*i))],'FontSize',16,'color',colorHolder((3*i-2):(3*i)));
        line([pks(i) pks(i)],[holdData(i)-15 maxVal],'color',colorHolder((3*i-2):(3*i)),'LineStyle','--','LineWidth',2);
    end
   q=gca;
   limY=q.YLim;
   limY(2)=limY(2)+40;
